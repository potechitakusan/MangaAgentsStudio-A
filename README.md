...
- コマ割りテンプレート集（全79種類）を原則使用し、演出上必要なページは理由を記録して独自配置にする。imagegenでは番号付きレイアウト画像を渡すページ全体の一括生成を基本に、配置・読み順を確認する。
...
制作を進めたあと、「次から字コンテ完成時の人物レビューを『指示』として登録して」のように伝えると、[プロセスチェッカー](docs/knowledge/process-checker.md)が追加方法を相談します。「指示」は進捗報告時に実施を照合し、「強く指示」は作品のAGENTS.mdにも反映します。必要ならHooksも追加できます。対象・条件・確認時点・根拠・引継ぎ範囲まで相談し、「おまかせ」ならCodexが決めます。

必須事項は作品内に保存され、再開時にも使います。リストが空なら詳細な照合は行いません。次の作品へ持ち越す場合は、会話で引継ぎ元を指定するか、作成時に次の引数を使います（引継ぎにはPython 3.10以上が必要です）。

```powershell
.\scripts\New-MangaProject.ps1 -ProjectName '新しい作品' -ProcessRequirementsFrom '../前の作品'
```

コピーするのは引継ぎ対象にした指示だけです。元作品の完了記録・素材・Hooks設定や信頼情報は含めません。引継ぎ元を指定しない新規作品は空のリストで始まり、既存作品は自動変更しません。
...
現在の運用では、既存パターンで意図した演出を十分に表現できないページは、Codexが理由を記録して独自配置を選べます。全ページで自由に設計したい場合は `OPTION.md` の「使用しない」、例外を認めず使いたい場合は「必須」を指定してください。imagegenではどちらの配置も番号付き画像として渡し、ページ全体を一括生成することを基本とします。詳細は [コマ割りの手順](docs/knowledge/panel-layout-policy.md) を参照してください。
