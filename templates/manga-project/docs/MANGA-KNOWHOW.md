...
説明は日本語。ファイル名、製品名、出典URL、機械用の識別子はそのまま記載する。映画・脚本の知識は必要な場面の補助に使う。
...
| 自分が必須にした工程を次回から照合したい | [プロセスチェッカー](knowledge/process-checker.md) | `config/process-requirements.json`、`.work/process-checker/` |
...
| R15 | [無言の種類と働きを確かめる](knowledge/manga/03-beat-pacing-timing.md#r15) |
...
本文は共通知識、記入用雛形とこの一覧は作品テンプレートで管理する。作品作成スクリプトが必要な文書をすべて同梱する。
