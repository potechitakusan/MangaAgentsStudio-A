...
2. 画風が未指定なら `config/novelai-style-presets.json` の標準候補（現在11件）を名前・特徴とともに提案する。既に指定されている場合は再選択を強要しない。
3. 見本はキットの `resources/novelai-style-samples/index.html`。プロジェクトに画像をコピーしない。既知のキット設置先から実在を確かめ、ユーザーに実際の絶対パスを案内する。設置先不明ならこの相対場所と未確認であることを伝え、場所を確認する。存在しないパスを作り話で補わない。
4. 選択回答を待って作品の採用IDとプロンプトを記録する。`default_ids` は提案候補であり、全画風の混合や先頭候補の無断自動選択ではない。特定作家・漫画家の名前は使わない。追加5候補の `sample_prompt_prefix` は見本用の人物数指定であり、実作品で全コマを女性2人に固定する規則ではない。
...
オノマトペは `templates/onomatopoeia/catalog.json` を意味・読み・用途・強さ・書体・反復数で検索する。適切なものがなければ別の透過素材を生成し、実alpha・文字・反復数を確認する。反復の1文字連結は4〜7文字を別IDで扱う。セリフは原則縦書き・各列上詰めで組版し、枠・効果音・吹き出し・文字を分離する。
